...
from webb.utils.notify import notify
from webb.utils.paths import atomic_dump, scheme_data_dir, scheme_path
...
class Scheme:
    ...
    def __init__(self, json: dict[str, any] | None) -> None:
        if json is None:
            self._name = "tokyonight"
            self._flavour = "medium"
            self._mode = "dark"
    ...
    def _update_colours(self) -> None:
        if self.name == "dynamic":
            from webb.utils.material import get_colours_for_image
            ...
            try:
                ...
                if self.notify:
                    notify(
                        ...
                        "No wallpaper set. Please set a wallpaper via `webb wallpaper` before setting a dynamic scheme.",
                    )
                raise ValueError(
                    "No wallpaper set. Please set a wallpaper via `webb wallpaper` before setting a dynamic scheme."
                )
