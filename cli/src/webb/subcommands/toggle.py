...
from webb.utils import hypr
from webb.utils.paths import user_config_path
...
class Command:
    args: Namespace
    cfg: dict | DeepChainMap
    clients: list | None = None

    def __init__(self, args: Namespace) -> None:
        self.args = args

        # Default workspace configs matching the shell's Icons.qml workspace names.
        # Override or extend any of these in ~/.config/webb/cli.json under "toggles".
        # Apps with "enable": false are skipped; set to true to activate them.
        self.cfg = {
            "communication": {
                "discord": {
                    "enable": False,
            ...
            "sysmon": {
                "btop": {
                    ...
                    "command": ["kitty", "--class", "btop", "--title", "btop", "-e", "btop"],
                },
            },
            "todo": {
                "todoist": {
    ...
    def move_client(self, selector, workspace: str) -> None:
    ...
    def spawn_client(self, selector, spawn: list[str]) -> bool:
    ...
    def handle_client_config(self, client: dict) -> bool:
        def selector(c: dict) -> bool:
            # Each match entry is OR'd; keys within a match entry are AND'd
